...
__PACKAGE__->attr('string_replace' => '%INSERT_GALLERY_HERE%');
...
__PACKAGE__->attr('imagetypes' => join( '|',  keys %content_types  ) );#  build the list of valid image types
...
sub hook_finalize {
	...
	my $gallery_name = $c->stash('year'). $c->stash('month').$c->stash('article')->{day}.'-'.$c->stash('alias');
	my $gallerydir = $publicdir .  '/'. $gallery_name;
	return unless (-d $gallerydir);#not gallery article
	...
	$self->print_gallery_thumbs($c, {'gallerydir' => $gallerydir,
									 'publicdir' => $publicdir,
...
sub print_gallery_thumbs
{
	...
	foreach my $img (@all_imgs)
	{
    	...
    	$self->_cache_image($c, {'source_file' =>  "$opts->{gallerydir}/$img",
    	...
    	push (@images, { 'thumbnail_url' =>  $thumbnail_url, 'large_url' => $large_url});
	}
	$c->stash('images' => \@images);
 	$c->stash('columns' => $self->columns, 'padding' => $self->padding, 'bgcolor' => $self->bgcolor);
 
    my $gallery_html = $c->render_partial('gallery', template_class => 'Bootylicious::Plugin::Gallery');
	my $body = $c->res->body; 
    my $str_replace = $self->string_replace;
...
sub _create_thubnail
{
	...
	$t->thumbnail(input   => $opts->{source_file},
...
L<Bootylicious::Plugin::Gallery> - Gallery plugin for Bootylicious (One-file blog engine software on Mojo steroids!)
 

=head1 INSTRUCTION


=head3 Register gallery plugin

Register gallery plugin in a configuration file (bootylicious.conf), add line like this:
	
	plugins=gallery:thumb_width=144:thumb_height=144:bgcolor=white
	

=head3 Create post  (named like this: 20090903-my-super-gallery.pod):
...
=head3 Create directory with foto 

 Create directory with fotos in publicdir (see in bootylicious.conf  publicdir=...) named like this: 20090903-my-super-gallery (same as post)
...
=head2 C<string_replace>

default '%INSERT_GALLERY_HERE%'
...
default 'white'
Background color of the thumbnail canvas (will only show if the ratio of the source does not match the ratio of the thumbnail).Default: white
...
L<http://getbootylicious.org>  
 One-file blog engine software on Mojo steroids!
